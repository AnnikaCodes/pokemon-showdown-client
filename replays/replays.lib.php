...
class Replays {
	...
	function init() {
		...
		if ($this->db) return;
		try {
			$arguments = "pgsql:host={$this->config['server']};port=5432;";
			$arguments .= "dbname={$this->config['database']};";
			$arguments .= "user={$this->config['username']};";
	...
	function get($id, $force = false) {
		...
		$res = $this->db->prepare("SELECT *, extract(epoch from uploadtime) as uploadtime FROM replays WHERE id = ? LIMIT 1");
		$res->execute([$id]);
		if (!$res) return [];
		$replay = $res->fetch();
		...
		// 	$res = $this->db->prepare("UPDATE replays SET views = views + 1, `password` = ? WHERE id = ?");
	...
	function search($args) {
		...
		if ($args["username"] ?? null) {
			...
			if ($args["username2"] ?? null) {
				...
				if ($format) {
					$res = $this->db->prepare("(SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? AND p2id = ? AND format = ? ORDER BY $order DESC) UNION (SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? AND p2id = ? AND format = ? ORDER BY $order DESC) ORDER BY $order DESC LIMIT 51 OFFSET $limit1;");
					$res->execute([$isPrivate, $userid, $userid2, $format, $isPrivate, $userid2, $userid, $format]);
				} else {
					$res = $this->db->prepare("(SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? AND p2id = ? ORDER BY $order DESC) UNION (SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? AND p2id = ? ORDER BY $order DESC) ORDER BY $order DESC LIMIT 51 OFFSET $limit1;");
					$res->execute([$isPrivate, $userid, $userid2, $isPrivate, $userid2, $userid]);
				}
			} else {
				if ($format) {
					$res = $this->db->prepare("(SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? AND format = ? ORDER BY $order DESC) UNION (SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p2id = ? AND format = ? ORDER BY uploadtime DESC) ORDER BY $order DESC LIMIT 51 OFFSET $limit1;");
					$res->execute([$isPrivate, $userid, $format, $isPrivate, $userid, $format]);
				} else {
					$res = $this->db->prepare("(SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p1id = ? ORDER BY $order DESC) UNION (SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND p2id = ? ORDER BY uploadtime DESC) ORDER BY $order DESC LIMIT 51 OFFSET $limit1;");
		...
		$res = null;
		if ($byRating) {
			$res = $this->db->prepare("SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, rating, password FROM replays WHERE private = ? AND formatid = ? ORDER BY rating DESC LIMIT 51 OFFSET $limit1;");
		} else {
			$res = $this->db->prepare("SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2, password FROM replays WHERE private = ? AND formatid = ? ORDER BY uploadtime DESC LIMIT 51 OFFSET $limit1;");
		}
	...
	function fullSearch($term, $page = 0) {
		...
		// $res = $this->db->prepare("SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2 FROM replays WHERE private = 0 AND (p1id = ? OR p2id = ?) ORDER BY uploadtime DESC LIMIT ?, 51");
		// $this->db->query("SET SESSION max_execution_time=5000");
		$res = null;
		switch (count($patterns)) {
		case 1:
			$res = $this->db->prepare("SELECT /*+ MAX_EXECUTION_TIME(10000) */ uploadtime, id, format, p1, p2, password FROM replays WHERE private = 0 AND log LIKE ? ORDER BY uploadtime DESC LIMIT 10;");
			$res->execute($patterns);
			break;
		case 2:
			$res = $this->db->prepare("SELECT /*+ MAX_EXECUTION_TIME(10000) */ uploadtime, id, format, p1, p2, password FROM replays WHERE private = 0 AND log LIKE ? AND log LIKE ? ORDER BY uploadtime DESC LIMIT 10;");
			$res->execute($patterns);
			break;
		default;
			return [];
	...
	function recent() {
		if (!$this->db) return [];
		$res = $this->db->query("SELECT extract(epoch from uploadtime) as uploadtime, id, format, p1, p2 FROM replays WHERE private = 0 ORDER BY uploadtime DESC LIMIT 50");
	...
	function prepUpload(&$reqData) {
		...
		$res = $this->db->prepare(
			"INSERT INTO prepreplays (id,loghash,p1,p2,format,private,uploadtime,rating,inputlog)
				VALUES (:id,:loghash,:p1,:p2,:format,:private,to_timestamp(:uploadtime),:rating,:inputlog)
			ON CONFLICT (id) DO
				UPDATE SET (id,loghash,p1,p2,format,private,uploadtime,rating,inputlog) = (:id,:loghash,:p1,:p2,:format,:private,to_timestamp(:uploadtime),:rating,:inputlog)"
		);
		$out = !!$res->execute([
			':id' => $id,
			':loghash' => $loghash,
		...
		// $res = $this->db->prepare("REPLACE INTO prepreplays (id,loghash,p1,p2,format,private,uploadtime,rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
	...
	function upload(&$reqData) {
		...
		$res = $this->db->prepare("SELECT *, extract(epoch from uploadtime) as uploadtime FROM prepreplays WHERE id = ?");
		if (!$res->execute([$id])) {
		...
		$res = $this->db->prepare("SELECT id, password, private FROM replays WHERE id = ?");
		$res->execute([$id]);
		$replay = $res->fetch();
		...
		$res = $this->db->prepare(
			"INSERT INTO replays (id, p1, p2, format, p1id, p2id, formatid, uploadtime, private, rating, log, inputlog, password) " .
			"VALUES (?, ?, ?, ?, ?, ?, ?, to_timestamp(?), ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET log = ?, inputlog = ?, rating = ?, private = ?, password = ?");
		...
		$res = $this->db->prepare("DELETE FROM prepreplays WHERE id = ? AND loghash = ?");
